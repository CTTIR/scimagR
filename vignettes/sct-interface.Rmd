...
vignette: >
  %\VignetteIndexEntry{SCT Interface}
...
scimagR provides R wrappers for the Spinal Cord Toolbox (SCT) command-line
tools. These wrappers handle:
- Path resolution (respects `SCT_DIR` environment variable)
- Input validation
- Idempotent execution (skip if output exists)
...
- **"Cannot find sct_deepseg on PATH"**: Set `SCT_DIR` or add SCT `bin/` to PATH
- **Segmentation fails silently**: Check stderr output in the returned result
